...
async function getGitHubAppToken(): Promise<string | undefined> {
  ...
  if (!privateKey.includes('-----BEGIN')) {
    ...
    const resolvedPath = path.isAbsolute(privateKey)
      ? privateKey
  ...
  try {
    ...
    console.warn(chalk.yellow(`Failed to authenticate with GitHub App: ${errorMessage}`));
    if (errorMessage.includes('error:1E08010C')) {
      console.warn(
        chalk.yellow(
          'Hint: The private key format seems incorrect. Ensure it is a valid PEM key with proper newlines.'
        )
      );
    }
    return undefined;
