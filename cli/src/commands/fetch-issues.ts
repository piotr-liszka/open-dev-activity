...
import { FetchIssuesOptions, ProjectV2Item, ProcessedIssue, TimelineEvent, StatusDuration } from '../types.js';
...
export const fetchIssuesCommand = new Command('fetch-issues')
    ...
    .action(async (options) => {
        try {
            ...
            console.log(chalk.blue(`Fetching issues for ${options.owner}/Project-${options.projectNumber}...`));
            ...
            processedItems.forEach(item => {
                console.log(chalk.whiteBright.bold(`[${item.status}] `) + chalk.cyan(`#${item.number} ${item.title}`));
                ...
                if (item.history.length > 0) {
                    ...
                    const sortedHistory = [...item.history].sort((a, b) => new Date(b.when).getTime() - new Date(a.when).getTime());
...
function processItem(item: ProjectV2Item, reportToDate: dayjs.Dayjs = dayjs()): ProcessedIssue {
    ...
    const history: import('../types.js').IssueHistoryItem[] = [];
