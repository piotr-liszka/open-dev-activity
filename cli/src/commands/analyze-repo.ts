...
export const analyzeRepoCommand = new Command('analyze-repo')
  ...
  .option(
    '--from <date>',
    'Start date to filter commits (YYYY-MM-DD or "7 days ago")',
    '7 days ago'
  )
...
async function fetchAllRepositories(
  ...
  while (hasNextPage) {
    ...
    const result: RepositoryResponse = await (client as any).graphqlWithAuth(repoQuery, {
      owner,
      cursor,
    });
    ...
    if (detectedForks.length > 0) {
      console.log(
        chalk.gray(`  - Initial fetch contains ${detectedForks.length} forked repositories.`)
      );
      ...
      console.log(
        chalk.yellow(
          `  - Warning: No forked repositories were found in the initial fetch for "${owner}".`
        )
      );
    }
    ...
    if (includeForks) {
      for (const repo of repoData.nodes) {
        ...
        try {
          const forkResult: RepositoryResponse = await (client as any).graphqlWithAuth(forkQuery, {
            owner,
            repo: repo.name,
          });

          console.log('forkQ', forkResult, {
            owner,
            repo: repo.name,
          });
          throw 'sads';
    ...
    console.log(
      chalk.gray(
        `Processing ${allRepoNodes.length} candidate repositories (including descendant forks: ${allRepoNodes.length - repoData.nodes.length})...`
      )
    );
    ...
    for (const repo of allRepoNodes) {
      ...
      if (activeBranches.length === 0) {
        ...
        const lastUpdated =
          repo.refs.nodes.length > 0 &&
          repo.refs.nodes[0].target?.history?.nodes?.[0]?.committedDate
            ? dayjs(repo.refs.nodes[0].target.history.nodes[0].committedDate).format('YYYY-MM-DD')
            : 'never';
      ...
      for (const branch of activeBranches) {
        ...
        try {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const branchResult: RepositoryResponse = await (client as any).graphqlWithAuth(
            branchQuery,
            {
              owner: repoOwner,
              repo: repoName,
          ...
          console.error(
            chalk.dim(
              `  Warning: Could not fetch commits from ${repo.nameWithOwner}:${branch.name}`
            )
          );
        }
      }
...
function displayRepositoryCommits(
  ...
  console.log(
    `  ${chalk.bold('Lines Added:')} ${chalk.green(`+${totalAdditions.toLocaleString()}`)}`
  );
  console.log(
    `  ${chalk.bold('Lines Deleted:')} ${chalk.red(`-${totalDeletions.toLocaleString()}`)}`
  );
  console.log(
    `  ${chalk.bold('Net Change:')} ${(totalAdditions - totalDeletions).toLocaleString()}`
  );
  ...
  allCommits.forEach((commit, index) => {
    ...
    const branchInfo =
      commit.branches.length > 0 ? chalk.dim(`[${commit.branches.join(', ')}]`) : '';

    console.log(
      `${chalk.gray(commitNumber)} ${chalk.yellow(commit.sha)} ${chalk.blue(commit.author)} ${diffStats} ${branchInfo}`
    );
